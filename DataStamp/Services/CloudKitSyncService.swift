...
/// CloudKit sync service for DataStamp items with full file sync
@MainActor
final class CloudKitSyncService: ObservableObject {
    
    // MARK: - Constants
    
    private let containerIdentifier = "iCloud.com.makiavel.datestamp"
    private let recordType = "DataStampItem"
    private let subscriptionID = "datestamp-changes"
    ...
    private let logger = Logger(subsystem: "com.makiavel.datestamp", category: "CloudSync")
    ...
    private func uploadItem(_ item: DataStampItem, to database: CKDatabase) async throws {
    ...
    private func processRemoteRecord(_ record: CKRecord, context: ModelContext) async throws {
        ...
        guard let contentTypeRaw = record["contentType"] as? String,
              ...
              let status = DataStampStatus(rawValue: statusRaw),
        ...
        if let existing = existing {
            ...
            // Create new
            item = DataStampItem(contentType: contentType, contentHash: contentHash)
            item.id = itemId
